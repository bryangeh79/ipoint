...
import {
  ...
  gteDecimal,
  lteDecimal,
} from './wallet-adjustment.owner.service.js';
import type {
...
/**
 * P7 SEC-01 iPoint adjustment owner unit tests.
 *
 * The owner is the SOLE enforcement boundary for every manual-iPoint
 * control (P7-OD-03/10/11/18/20): permission re-check, selected market,
 * caps routing (10,000/100,000 Malaysia baseline, no fallback), evidence
 * and attachment rules, reason-code catalog, immutable rejected
 * replacement linkage, payload-hash idempotency, Maker/Checker inequality
 * and the exact-decimal helpers. The full transaction paths (lifecycle
 * transitions, atomic ledger + state + audit, concurrency, failure/retry)
 * are exercised by the integration suite on fresh PostgreSQL databases.
 */
...
let service: WalletAdjustmentOwnerService;
...
const ACTOR: WalletAdjustmentOwnerActor = {
...
/**
 * Mock the pre-transaction lookups: wallet (member_wallet_accounts),
 * active market (markets), market rule, reason code and the prior-request
 * linkage check (ipoint_adjustment_requests). Keyed by drizzle table name.
 */
...
function createService(rowsByTable: Record<string, unknown[]>) {
  ...
  service = new WalletAdjustmentOwnerService(
...
describe('exact-decimal helpers (string-only, never float)', () => {
  it('compares decimals exactly', () => {
    ...
    expect(gteDecimal('10000.0000000001', '10000')).toBe(true);
    expect(gteDecimal('10000', '10000.0000000001')).toBe(false);
  });

  it('adds signed decimals exactly', () => {
    expect(addDecimal('10000', '1')).toBe('10001');
    expect(addDecimal('0.5', '0.25')).toBe('0.75');
    expect(addDecimal('100', '-50')).toBe('50');
    expect(addDecimal('1.0000000000', '-0.0000000001')).toBe('0.9999999999');
  });
...
describe('create: identity and market guards', () => {
  ...
  it('rejects when no server Current Admin Market is selected', async () => {
    ...
    await expect(
      ...
      code: 'WALLET_ADJUSTMENT_MARKET_SELECTION_REQUIRED',
    });
  });
  ...
  it('rejects an invalid amount grammar and zero', async () => {
    ...
    for (const amount of ['0', '0.00', '-5', '1.00000000001', 'abc']) {
      ...
      ).rejects.toMatchObject({ code: 'WALLET_ADJUSTMENT_INVALID_AMOUNT' });
    }
  });
...
describe('create: wallet / market / caps / evidence guards', () => {
  beforeEach(() => {
    createService({
      ['member_wallet_accounts']: [WALLET_ROW],
      ['markets']: [MARKET_ROW],
      ['ipoint_adjustment_market_rules']: [RULE_ROW],
      ['ipoint_adjustment_reason_codes']: [LOW_RISK_REASON],
    });
  });

  it('rejects an unknown or archived wallet', async () => {
    const { db } = createService({ ['member_wallet_accounts']: [] });
    void db;
    await expect(service.create(ACTOR, createCommand())).rejects.toMatchObject({
      code: 'WALLET_ADJUSTMENT_WALLET_NOT_FOUND',
    });
  });

  it('rejects when the wallet market differs from the Current Admin Market', async () => {
    await expect(
      service.create(
      ...
      code: 'WALLET_ADJUSTMENT_MARKET_CONTEXT_MISMATCH',
    });
  });
  ...
  it('rejects an inactive or unknown reason code', async () => {
    ...
    ).rejects.toMatchObject({ code: 'WALLET_ADJUSTMENT_REASON_CODE_INVALID' });
  });

  it('requires an attachment reference above the soft cap', async () => {
    await expect(
      service.create(ACTOR, createCommand({ amount: '10001' })),
    ).rejects.toMatchObject({ code: 'WALLET_ADJUSTMENT_ATTACHMENT_REQUIRED' });
  });
  ...
  it('rejects a prior-request linkage that is not a rejected request', async () => {
    ...
    await expect(
      ...
      code: 'WALLET_ADJUSTMENT_PRIOR_REQUEST_INVALID',
    });
  });
});
...
describe('decide: Maker/Checker inequality and caps routing (unit guards)', () => {
  it('denies the maker as checker at any amount', async () => {
    ...
    ).rejects.toMatchObject({ code: 'WALLET_ADJUSTMENT_PERMISSION_DENIED' });
  });
  ...
  it('requires a decision reason', async () => {
    ...
    await expect(
      ...
      code: 'WALLET_ADJUSTMENT_DECISION_REASON_REQUIRED',
    });
  });
});
