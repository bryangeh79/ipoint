 ...
 *
 * Rate-limit methodology: buckets of the in-memory auth rate limiter are
 * cleared between measured blocks (P2-S4D / P4-S7 precedent) so the measured
 * endpoint cost excludes artificial 429s; the limiter's own 429 behaviour is
 * covered by the auth integration suites.
 *
...
export async function runJourneyJ1(ctx: LoadContext): Promise<JourneyResult> {
  ...
  let loginIndex = 0;
  await measureOp(ctx, result, 'login', OK, async () => {
  ...
  clearRateLimiter(ctx);
  let refreshIndex = 0;
  await measureOp(ctx, result, 'refresh', OK, async () => {
  ...
  clearRateLimiter(ctx);
  let otpIndex = 0;
  await measureOp(ctx, result, 'otp/issue', CREATED, async () => {
  ...
  await measureOp(ctx, result, 'otp/verify', OK, async () => {
    const actor = pool[otpIndex % pool.length] ?? defaultActor;
    otpIndex += 1;
    ...
    const body = issued.body as { otp_id?: string; development_code?: string };
    return httpCall(ctx.baseUrl, {
      method: 'POST',
  ...
  if (ctx.level === 'L2') {
    clearRateLimiter(ctx);
    let logoutIndex = 0;
    await measureOp(ctx, result, 'logout', NO_CONTENT, async () => {
