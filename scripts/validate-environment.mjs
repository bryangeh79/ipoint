...

{
  ...
  if (r.exitCode === 0) {
    record(
      'node22 binary (project min)',
      'PASS',
      'Node ' + r.stdout + ' \u2014 meets >=22.0.0 requirement',
    );
  } else {
    record(
      'node22 binary (project min)',
      'FAIL',
      'Missing or broken \u2014 project requires >=22.0.0',
    );
  }
}
...
);
{
  ...
  if (r.exitCode === 0) {
    ...
    record(
      'pnpm (PATH)',
      'FAIL',
      'Not in PATH \u2014 need pnpm@9.15.9 per package.json',
    );
  }
}
  ...
  if (existsSync(pnpmBin)) {
    ...
    record(
      ...
      'File exists (' +
        ...
        ' MB), ELF binary: ' +
        isELF +
        '. Cannot execute \u2014 causes SIGKILL in sandbox (likely incompatible glibc or Node.js mismatch)',
    );
  } else {
...

{
  record(
    'pnpm via node22',
    'SKIP',
    'Standalone binary \u2014 cannot be run via Node.js',
  );
}
...
);
{
  ...
  if (r.exitCode === 0) {
    ...
    record(
      'corepack enable',
      'FAIL',
      'Exit ' + r.exitCode + ': ' + (r.stderr || r.stdout),
    );
  }
}

{
  try {
    ...
    if (r.exitCode === 0) {
      ...
      if (pnpmR.exitCode === 0)
        record('pnpm via corepack', 'PASS', 'pnpm ' + pnpmR.stdout);
      else
        record(
          'pnpm via corepack',
          'FAIL',
          'corepack prepared but pnpm still fails: ' + pnpmR.stderr,
        );
    } else {
      record(
...
);
{
  ...
  if (existsSync(nm)) {
    ...
    record(
      'node_modules exists',
      'FAIL',
      'Root node_modules directory missing',
    );
  }
}
...
for (const pkg of ['prettier', 'vitest', 'eslint', 'typescript']) {
  ...
  if (!existsSync(p)) {
    record(
      'symlink: ' + pkg,
      'FAIL',
      'Missing \u2014 not resolved in node_modules',
    );
    continue;
  }
  const s = lstatSync(p);
...
function scanDir(dir) {
  ...
  for (const e of entries) {
    if (e === '.bin' || e === '.pnpm' || e === '.modules.yaml' || e === '.vite')
      continue;
    const fp = join(dir, e);
    let s;
    try {
...
if (existsSync(pnpmStore)) {
  ...
  record(
    '.pnpm virtual store',
    'PASS',
    'Found with ' + entries.length + ' package entries',
  );
} else {
...
);
{
  ...
  if (existsSync(storeDir)) {
    ...
    record(
      ...
      'PASS',
      'Found at ' + storeDir + ' with ' + entries.length + ' entries',
    );
  } else {
    record(
      'pnpm store directory',
      'FAIL',
      'Missing \u2014 store may be elsewhere',
    );
  }
}
  ...
  if (r.exitCode === 0) {
    ...
    if (existsSync(join(ROOT, '.git'))) {
      record(
        ...
        'INFO',
        'Repo exists but git CLI unavailable',
      );
    }
  }
}
...
);
{
  ...
  if (dbUrl) {
    ...
    if (r.exitCode === 0)
      record('pg_isready', 'PASS', 'PostgreSQL accepting connections');
    else
      record(
        'pg_isready',
        'FAIL',
