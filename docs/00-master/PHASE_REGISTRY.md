...
| **P7-S6** | Commercial Configuration (Merchant Package / Reward / Redemption Rate / Commission) | **S6A DELIVERED (provisional-complete)** · **S6B PROVISIONAL_LOCAL_INTEGRATION / CG-02_BLOCKED / WRITE_SURFACE_NOT_AUTHORIZED** (gate revoked under Command Center order 2026-08-04; D-052 remediation mandatory) · **S6C PAUSED_DEPENDENCY_GATE** (implementation preserved on task/p7-s6c-redemption-config, NOT integrated) · S6D NOT_AUTHORIZED until D-052 | Continuous execution under D-047/D-048; D-050 (D-052) gate must pass before S6B rewiring and S6C/S6D resume. |
...
| **fix/p3-p7-reward-rule-owner** | Phase 3 Reward-Rule Owner Security/Versioning Remediation (D-052, Command Center "D-050") | **AUTHORIZED — EXECUTE NOW** | Mandatory under Command Center order 2026-08-04 §6-§7; CG-02 gate; independent implementer + reviewer + integration verifier; S6B rewiring and S6C/S6D resume only after acceptance. |
