...
## D-081 - L-06 /members/me/qr CLOSED: bounded implementation integrated (D-080 executed)

| Field | Value |
|---|---|
| **Decision ID** | D-081 |
| **Date** | 2026-08-11 |
| **Source** | OpenClaw (D-079: succeeded Command Center, revocable); Bryan Option A decision (D-080) executed |
| **Old Rule** | L-06 gap OPEN (OBS-07/D-074): /members/me/qr surface absent from the API; P8-S9 gate rows G-30/G-31 blocked on the QR decision |
| **New Decision** | **L-06 CLOSED - bounded implementation integrated** (merge `50a0ac94` on `phase/8-final-delivery-readiness`; implementer commit `59ce67c0` on `fix/p8-l06-member-qr`; 13 files +2378 lines). Surface delivered per the frozen Phase 2 contract: GET /members/me/qr (MemberQrResponse; no row -> 200 {qr:null}), POST /members/me/qr (rotate, single transaction, old ACTIVE -> ROTATED + rotated_to_id chain, new ACTIVE + rotated_from_id), DELETE /members/me/qr (revoke -> 204; already revoked -> QR_REVOKED). L-06/D-14 compliance: HMAC-SHA256 signed self-contained short-lived display token (default 300s, MEMBER_QR_TTL_SECONDS 1..86400 validated); claims = public_qr_id + issued_at + expires_at + nonce only (no internal IDs/email/phone); plaintext token NEVER persisted (SHA-256 hex 64-char hash, matches CHECK constraint); rotation/revocation+audit+idempotency record in one transaction; FOR UPDATE row-lock + partial-unique-index (one ACTIVE per member) invariant; idempotency via header/body dual channel (IDEMPOTENCY_CONFLICT); audit on all three routes; errors mapped to contract codes (no raw DB error leakage); MEMBER_QR_SIGNING_SECRET env-configured (min 32, fail-closed, no hard-code). Zero-owner-bypass re-scan: 0 direct bypass (13 pattern hits all pre-existing documented items). Evidence: typecheck/build PASS, eslint 0, prettier PASS, unit 26/26, integration 20/20 (fresh ipoint_p8l06_test DB, P8L06_DESTRUCTIVE_TEST=1, fail-closed guard verified), openapi:validate PASS at 301 paths (0 dup operationId, 0 missing schemas), packages/database/** zero change, checksums 40/40 frozen, main 69240bf8 untouched. Independent reviewer B' (D-060 pool) APPROVED 0C/0H/0M with 7 recorded LOW/INFO (L-1 delivery-note wording vs service behavior on expired display token - non-blocking; L-2 STATE_CONFLICT fallback not in GET error list - unreachable in normal flow; L-3 GET MEMBER_NOT_FOUND path un-audited - matches member-self pattern; L-4 idempotency response cache holds self-contained display payload not raw secret; L-5 concurrent same-key race may surface QR_ACTIVE_EXISTS; L-6 .local env points at default DB - test DB must be explicit; L-7 Drizzle mapping already existed at packages/database/schema/index.ts:1294 - brief errata, zero compliance impact). api-client/member-web zero change (member-web QR display = P9 follow-up per brief). P8-S9 gate rows G-30/G-31 QR item now consumable as GREEN; the final gate executes on this integrated state. |
| **Reason** | D-080 Option A executed with full evidence chain (implementer + independent review + host integration gate + zero-bypass re-scan); the last LOCKED-rule gap before the final gate is closed, enabling an unconditional 0 unresolved HIGH declaration at P8-S9; all constraints honored (no migration, no frozen-owner touch, no plaintext token persistence, no sensitive exposure). |
| **Affected Files** | apps/api/src/member-qr/** (new), apps/api/src/app.module.ts (+2), .env.example (+6), docs/06-phase-reports/p8-s9/P8_S9_L06_DELIVERY_NOTE.md, docs/06-phase-reports/p8-s9/TASK_BRIEF_P8S9_L06.md, docs/00-master/DECISION_LOG.md (this entry), docs/00-master/PHASE_REGISTRY.md |
| **Affected Phases** | Phase 8 (L-06 CLOSED; P8-S9 final gate next on integrated state) |
| **Migration** | NONE (checksums 40/40 frozen) |
...
## D-083 - IPOINT V1 ENGINEERING COMPLETE: Phase 8 closed and frozen; gaps recorded truthfully (Bryan directive 2026-08-12)

| Field | Value |
|---|---|
| **Decision ID** | D-083 |
| **Date** | 2026-08-12 |
| **Source** | Bryan (final business decision owner) - webchat directive 2026-08-12 15:16 MYT: "让 iPoint V1 工程在此收口，缺口如实入档" (close iPoint V1 engineering at this point; record all gaps truthfully into the archive). Executed by OpenClaw (D-079: succeeded Command Center incl. final acceptance authority, revocable). |
| **Old Rule** | Phase 8 delivered (P8-S9 GATE GREEN 32/32, D-082; P8-S10 final delivery report `b3e3838a`); `PHASE_8_ACCEPTED/COMPLETE/CLOSED/FROZEN/IPOINT_V1_ENGINEERING_COMPLETE` declared NOT issued, reserved for OpenClaw per D-079 after the P8-S10 report; UI-gap disposition (repair vs formal deferral) left open per P8-S10 sec.3.1.1; OPEN_QUESTIONS O-13 status stale (OPEN despite Phase 7 remediation); EXECUTOR_PROVENANCE_REGISTER missing P8-S10 entry; PROJECT_MASTER_CONTROL role table not yet reflecting D-079. |
| **New Decision** | **(1) CLOSURE DECLARATIONS ISSUED:** `PHASE_8_ACCEPTED` / `PHASE_8_COMPLETE` / `PHASE_8_CLOSED` / `PHASE_8_FROZEN` / `IPOINT_V1_ENGINEERING_COMPLETE`. iPoint V1 engineering is complete and frozen at branch `phase/8-final-delivery-readiness` (final gate evidence: P8-S9 32/32 GREEN, unmet list none, 0 unresolved CRITICAL / 0 unresolved HIGH; `main` `69240bf8` untouched). Final progress values stand as P8-S10 recomputed: SELLABLE_DELIVERABLE_PROGRESS **93.75%** (94% rounded; below 100% for the exact three UI gaps below), PRODUCTION_READY_V1_PROGRESS **100%**. **(2) GAP DISPOSITION - FORMAL ACCEPTANCE-WITH-LIMITATION (record + defer, NOT repair):** per the Bryan closure directive, the three sellable-domain UI gaps are formally accepted as V1 known limitations and archived in `IPOINT_V1_CLOSURE_RECORD.md` (UI-1 Domain 17 reconciliation admin-web management UI; UI-2 Domain 18 risk/fraud admin-web review UI; UI-3 Domain 10 merchant reversal/refund UI - all backend/API complete and green, UI surfaces out of delivered scope) -> moved to Phase 12 deferred backlog, future work requires a new explicit decision. UI-state observations OBS-06/OBS-07/OBS-10 + member-web QR display UI recorded as P9-class triage (no backend/financial/security impact). Accepted-risk items unchanged: SEC-01 6 HIGH advisories (D-075 Bryan written acceptance, re-review at production-launch gate), D-078 L-2 hygiene artifact (cleanup requires explicit approval). **(3) ARCHIVE-HYGIENE CORRECTIONS (this commit):** O-13 status corrected OPEN -> RESOLVED in OPEN_QUESTIONS.md (remediated in Phase 7, merge `bb8d6f01`, `O13_FINAL_GATE_RECORD.md`); P8-S10 report-writer entry appended to EXECUTOR_PROVENANCE_REGISTER.md; PROJECT_MASTER_CONTROL.md role table annotated per D-079 (Command Center unavailable since 2026-08-11; OpenClaw fully succeeds the role through V1 closure, revocable; business/legal/commercial remain Bryan-exclusive). **(4) POST-CLOSURE BOUNDARY (unchanged):** Main PR / Main Merge / Push Main / Production Deployment NOT_AUTHORIZED; production-launch strategy gate (incl. SEC-01 re-review, backup/PITR, Redis pool-acquire-timeout config) is a future Bryan-scoped decision point; Phase 12 deferred backlog not to be implemented without a new explicit decision. |
| **Reason** | Bryan explicitly instructed closure at this point with truthful gap archiving. All engineering acceptance preconditions were met (D-082 final gate, D-083 prerequisites: P8-S10 delivery report complete). The gaps are scope-of-UI limitations with complete green backends - recording them truthfully as accepted limitations (rather than silently carrying them or falsely claiming 100% sellable) honors the D-058 sec.26 truthful-reporting requirement and the Bryan directive. Archive-hygiene corrections keep the governance record internally consistent at the moment of freeze. |
| **Affected Files** | docs/00-master/DECISION_LOG.md (this entry), docs/00-master/PHASE_REGISTRY.md, docs/00-master/OPEN_QUESTIONS.md (O-13 status), docs/00-master/PROJECT_MASTER_CONTROL.md (role table per D-079), docs/00-master/EXECUTOR_PROVENANCE_REGISTER.md (P8-S10 entry), docs/06-phase-reports/p8-s10/IPOINT_V1_CLOSURE_RECORD.md (new - closure record + gap archive) |
| **Affected Phases** | Phase 8 (CLOSED/FROZEN); iPoint V1 (ENGINEERING_COMPLETE); Phase 12 deferred backlog (UI-1/UI-2/UI-3 added) |
| **Migration** | NONE (checksums 40/40 frozen) |
| **Approver** | Bryan (closure directive 2026-08-12) + OpenClaw (D-079, revocable) |
| **Basis** | Bryan webchat directive 2026-08-12 15:16 MYT; D-079; D-082; PHASE_8_FINAL_DELIVERY_REPORT.md (`b3e3838a`) sec.3.1.1/sec.5; P8-S5a/S5c gate records; O13_FINAL_GATE_RECORD.md (merge `bb8d6f01`) |
| **Status** | **CLOSED (APPROVED)** |
