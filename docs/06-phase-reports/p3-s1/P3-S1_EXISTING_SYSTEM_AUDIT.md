...
```typescript
export const mcpLedgerAccounts = pgTable('mcp_ledger_accounts', {
  ...
  balance: numeric('balance', { precision: 38, scale: 10 })
    .notNull()
    .default('0'),
...
```typescript
export const mcpLedgerEntries = pgTable('mcp_ledger_entries', {
  id: uuid('id').primaryKey().defaultRandom(),
  accountId: uuid('account_id')
    .notNull()
    .references(() => mcpLedgerAccounts.id),
